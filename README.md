...
轻量化 SPT 拟真 Bot 行为模组。当前版本为 **0.1.7 开发验证版**：新增失视后的短暂守点与同区域再次探头识别。沿用 0.1.6 的有限撤离、挡枪换位、不可达点记忆及 Waypoints 前置。行为只使用 Bot 已确认的个人视觉，仍保留真实视线和射击安全校验；新版尚需游戏内实测，原生撤离层交接尚未解决。
...
构建依次核对本地游戏接口及 Waypoints 安装、编译 Release 工程、运行独立行为检查，最后输出 `dist/AiBehaviorOpt-0.1.7.zip`。仅打包两个自有 DLL 与说明文件，不修改游戏安装，不修改全局 SDK 环境。
...
0.1.7 新增开销评为 **低（静态分析预估，未实测）**：每个 Bot 仅增加固定大小视觉状态，守点转向每次失视最多提交一次，不增加射线或寻路。大量 Bot 同帧反复可见/失视时，边沿判断和限频日志可能集中执行；此前 0.1.5 海岸线数据仅作历史对照，不能代替新版测量。
