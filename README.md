...
轻量化 SPT 拟真 Bot 行为模组。当前版本为 **0.1.8 开发验证版**：新增近距目视交战时的一次有限主动侧移，改善 Bot 在开阔位置原地对枪的表现。沿用失视守点、受阻换位、掩体与受压反馈及 Waypoints 前置；仅使用 Bot 自己确认的玩家视觉，继续保留完整路径和射击安全校验。新版尚需局内实测。
...
构建依次核对本地游戏接口及 Waypoints 安装、编译 Release 工程、运行独立行为检查，最后输出 `dist/AiBehaviorOpt-0.1.8.zip`。仅打包两个自有 DLL 与说明文件，不修改游戏安装，不修改全局 SDK 环境。
...
0.1.8 新增开销评为 **低（静态分析预估，未实测）**：每个 Bot 只保存固定大小交战窗口状态，主动侧移每个连续交战窗口最多排队一次，沿用全局路径额度。多个 Bot 同时在六至二十五米与玩家目视交战且都缺少掩体时，短距路线可能集中排队；此前海岸线数据不能代替新版测量。
