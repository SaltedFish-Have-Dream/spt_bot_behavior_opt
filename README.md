...
轻量化 SPT 拟真 Bot 行为模组。当前版本为 **0.1.3 开发验证版**：在玩家专属行为基础上，修复受击压低与移动抬高姿态的指令冲突，增加普通姿态稳定窗口和掩体到达缓冲。已完成本地构建、39 组核心检查及 19 项接口核对，姿态修复等待局内复测。0.1.2 实测发现的搜索可达性问题仍待修复；尚未完成全面行为验收或 SAIN 性能对照。
...
构建依次核对本地游戏接口、编译 Release 工程、运行独立行为检查，最后输出 `dist/AiBehaviorOpt-0.1.3.zip`。仅打包两个自有 DLL 与说明文件，不修改游戏安装，不修改全局 SDK 环境。
...
当前改动运行开销预估 **低（静态分析，未局内实测）**：AI 来源在入口旁路，无玩家情境时只保留轻量检查；近弹使用固定队列与纯几何额度，不额外发射检测射线。玩家连射/霰弹同时威胁多名 Bot、复杂区域同步寻路及日志写入阻塞仍可能形成峰值，过载会丢弃近弹段并记录数量。0.1.0 海岸线旧日志与 AI Limit 负载不能代表本版或相对 SAIN 的收益。时间片是软预算，不是整局 AI 耗时上限。
