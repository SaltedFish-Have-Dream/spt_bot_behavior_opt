...
internal sealed class BotAgent : IDisposable
{
    ...
    {
        if (Disposed || Owner.BotState != EBotState.Active || !GameAdapter.DirectlyVisible(enemy, now)) return; // 停用回调和过期视觉不进入精确记忆。
    ...
    internal void ObserveSound(string identity, Vector3 position, double now)
    {
        ...
        if (Memory.TryGet(identity, now, out Observation previous) && now - previous.ObservedAt < 1) // 同一声源一秒内合并。
        { Runtime.Diagnostics.Count(DiagnosticEvent.SoundMerged); return; } // 事件风暴只留下计数证据。
        ...
        if (!InvestigationPolicy.InRange(Role, GameAdapter.Snapshot(Owner.Position), GameAdapter.Snapshot(position))) // 按实际带误差的调查点判断，避免接管后立即因超距退出。
        {
        ...
        if (Memory.Observe(observation, now)) // 合法新声音唤醒调查。
        {
            ...
            if (Trace(DiagnosticEvent.SoundSaved, now)) WriteTrace("SOUND_SAVED", now, FormattableString.Invariant($"error={error:F1} ttl={observation.ExpiresAt - now:F1}")); // 不输出声源真实身份与坐标。
        }
    }
    ...
    {
        if (_hasClue && !_visible && !Controlled && !InvestigationPolicy.InRange(Role, GameAdapter.Snapshot(Owner.Position), _clue.Position)) // 恢复或首次接管前验证声音与失去视线后的搜索范围。
        {
    ...
    internal void Claim()
    {
        if (Disposed || Controlled || Owner.BotState != EBotState.Active) return; // 停用与重复接管都不能重新提交动作。
    ...
    private void AimAndFire(double now)
    {
        ...
        if (!_visible || !TryVisibleObservation(enemy, now, out Observation sight) || enemy?.CanShoot != true || _recoveryRunning) // 同时验证目标身份、合法部位与最新观察。
    ...
    internal bool AllowShot(double now, int frame)
    {
        ...
        if (!Gate.CanFire(Skill, now)) return RejectShot(DiagnosticEvent.ShotReacting); // 等级反应门槛单独归因。
    ...
    internal bool VerifyShot(in WorkRequest request, double now, int frame)
    {
        ...
        if (Owner.BotState != EBotState.Active || !_shotPending || request.Generation != Generation || enemy == null || enemy.ProfileId != _shotIdentity || !TryVisibleObservation(enemy, now, out _) || !enemy.CanShoot)
    ...
    private void Search(double now)
    {
        ...
        if (_searchIndex >= 3 || !InvestigationPolicy.InRange(Role, GameAdapter.Snapshot(_searchStart), _clue.Position)) // 搜索完成或超出局部职责就结束。
        {
    ...
    {
        if (State != BehaviorState.Investigate && State != BehaviorState.Search) return; // 同帧重复回调不能再次清理。
