...
function Assert-ApiMethod {
    param($Module, [string]$Type, [string]$Name, [string]$Returns, [string[]]$Parameters)
    ...
    foreach ($apiMethod in $apiDefinition.Methods) {
        ...
        if ($apiMethod.ReturnType.FullName -ne $Returns -or $apiMethod.IsStatic -or -not $apiMethod.IsPublic) { throw "方法属性不匹配：$Type.$Name" } # 校验调用方式。
