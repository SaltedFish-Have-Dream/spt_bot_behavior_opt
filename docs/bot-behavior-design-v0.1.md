...
日期：2026-09-22。项目暂名：`ai_behavior_opt`。状态：0.1.0 完成 [首局海岸线日志实测](playtests/2026-09-22-shoreline.md)，随后已落地 [0.1.1 修复](v0.1.1-fixes.md)，等待局内复测，尚未完成性能对照；实际范围见 [首版实现记录](v0.1-implementation.md)，安装见 [使用说明](v0.1-user-guide.md)。
...
首版已建立独立客户端工程并实现单体行为、预算和回退，完成本地编译、纯逻辑与程序集接口检查。下一步按阶段验收条件进行游戏内 Brain 接入、感知与动作交接回归，再记录原生、本插件、SAIN 的对照数据；尚未完成的游戏内验收不能用构建通过替代。
